...
TARGET_PAGES = [
    ...
    # "https://www.facebook.com/tintucvietnammoinong",    # Yến 
...
def get_reactions_detail(page, post_el) -> dict:
    ...
    def _normalize_reaction_name(name: str):
        ...
        mapping = {
            ...
            "love": "Love", "yêu thích": "Love", "thương thương": "Love",
            "haha": "Haha", "cười": "Haha",
            "wow": "Wow", "ngạc nhiên": "Wow",
            "sad": "Sad", "buồn": "Sad",
            "angry": "Angry", "phẫn nộ": "Angry",
            "care": "Care", "thương": "Care",
        }
        ...
        for target_url in TARGET_PAGES:
            ...
            while collected < TARGET_POSTS and not stop_page:
                ...
                for btn in share_btns:
                    try:
                        handle = btn.evaluate_handle("""(el) => {
                            let node = el.parentElement;
                            for (let i = 0; i < 20; i++) {
                                if (!node) break;
                                const r = node.getBoundingClientRect();
                                if (r.width > 400 && r.height > 400) return node;
                                node = node.parentElement;
                            }
                            return null;
                        }""")
                        # as_element() trả về None nếu JS return null
                        post_el = handle.as_element()
                        if post_el is None:
                            continue
                        # Dedup element references
                ...
                for post_el in posts:
                    ...
                    try:
                        ...
                        warm_post_for_permalink(page, post_el)
                        quick_url = get_best_post_url_from_element(page, post_el, page_handle)
                        if not quick_url:
                            print(f"  {processed_count + 1}. [Bỏ qua]: chưa render được post_url")
                            continue
                        ...
                        if status != 'OK':
                            print(f"  {processed_count}. [Bỏ qua]: {status} | {text_preview_quick}")
                        ...
                        # --- Đủ điều kiện: mới extract đầy đủ (bao gồm mở modal reactions) ---
                        ...
                        if (
                            ...
                            print(f"  {processed_count}. [Bỏ qua]: không có metadata tương tác | {text_preview}")
                            continue

                        if post_data['total_reactions'] < MIN_REACTIONS:
                            print(f"  {processed_count}. [Bỏ qua]: Tương tác thấp  {post_data['total_reactions']} < {MIN_REACTIONS} | {text_preview}")
                            seen_urls.add(quick_url)
                            continue
                        ...
                        print(f"  {processed_count}. [THÀNH CÔNG {collected}/{TARGET_POSTS}]: {text_preview}")

                        # --- Kiểm tra nghỉ theo Batch ---
                        posts_in_batch += 1
                        if posts_in_batch >= BATCH_SIZE and collected < TARGET_POSTS:
                            sleep_time = random.randint(*BATCH_SLEEP)
                            print(f"\n[NGHỈ] Đã thu {posts_in_batch} bài. Nghỉ {sleep_time} giây...")
                            time.sleep(sleep_time)
                            posts_in_batch = 0

                    except Exception as e:
                        print(f"  {processed_count}. [LỖI]: {e}")
                        continue
            ...
            print(f"\n[HOÀN THÀNH] Đã thu {collected}/{TARGET_POSTS} bài viết từ {page_name}")
            sleep_next = random.randint(*PAGE_SLEEP)
            print(f"Chờ {sleep_next} giây trước khi sang trang tiếp theo...")
